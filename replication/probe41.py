"""probe41.py — symmetry breaking: two independent children (different inits) of the same parent (end of gen 0):
   ...
   python probe41.py -> results_v3/probe41.md"""
import glob, json, os
...
def parent(s):
    f = glob.glob(f"results_v3/*_seed{s}.json") + glob.glob(f"results_v3_confirm/*_seed{s}.json"); j = json.load(open(f[0])); r = [x for x in j["log"] if x["gen"] == 0][-1]; return np.array(r["language"]), np.array(r["decode"])
def job(args):
...
if __name__ == "__main__":
    raw = "results_v3/probe41_raw.json"
    if os.path.exists(raw): res = [tuple(x) for x in json.load(open(raw))]
    else:
        with Pool(8) as p: res = p.map(job, [(s, arm, i) for s in range(30) for arm in ("same", "different", "none") for i in (1, 2)])
        json.dump(res, open(raw, "w"))
    R = {(s, arm, i): np.array(l) for s, arm, i, l in res}; worlds = {s: World(3, 4, 0.25, seed=s) for s in range(30)}
    O = ["# probe41 — symmetry breaking: sibling partitions (30 seeds, two inits per arm)", "", "| arm | ARI sibling–sibling | ARI child–parent (mean of 2) | share of train forms shared by siblings |", "|---|---|---|---|"]
    V = {}
    ...
    open("results_v3/probe41.md", "w").write("\n".join(O) + "\n"); print("\n".join(O))
